...
export default function SupervisorCuidadores() {
  const [caregivers, setCaregivers] = useState([]);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState("");
  const [expanded, setExpanded] = useState({}); // { [caregiverId]: boolean }
  const [childrenByCaregiver, setChildrenByCaregiver] = useState({}); // { [caregiverId]: Child[] }
  const [loadingChildren, setLoadingChildren] = useState({}); // { [caregiverId]: boolean }
  ...
  useEffect(() => {
    async function getData() {
      try {
        setLoading(true);
        const response = await axios.get("/cuidador/meus-cuidadores-supervisor");
        const data = Array.isArray(response.data) ? response.data : [];
        setCaregivers(data);
        // Pré-indexa crianças por cuidador para expandir rapidamente
        const index = {};
        data.forEach((c) => {
    ...
    <Container>
      <Box sx={{ maxWidth: 1280, mx: "auto", p: 2 }}>
        <Breadcrumbs sx={{ mb: 2 }}>
          ...
          <Typography color="text.primary">Meus Cuidadores</Typography>
        </Breadcrumbs>
        <Box sx={{ mb: 3 }}>
          ...
          <Typography variant="body2" color="text.secondary">
            Supervisão de cuidadores e crianças vinculadas
          </Typography>
        </Box>
                                  ...
                                  <Card variant="outlined" sx={{ p: 1.5, borderRadius: 1 }}>
                                    <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                                      ...
                                      <Box sx={{ display: "flex", gap: 1.5 }}>
                                        ...
                                        <Tooltip title="Desligar do programa">
                                          <span>
                                            <Button
                                              variant="contained"
                                              color="error"
