...
export default function CriarTabelaParaGestante({ match }) {
  ...
  useEffect(() => {
    async function fetchChildData() {
      ...
      setChild(response.data.cuidador);
    }
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`/tabelas/create/gestante/${id}`, {
        dayOfVisit,
        period,
  ...
  return (
    <Div onSubmit={handleSubmit}>
      <h2>Criar tabela de visita para {child.name}</h2>
